...
{
	return graf[row][col] == 1;
}

Board::Board(bool graf[ROWS][COLUMNS])
{
	setStart(0, 0);
	...
	for (int i = 0; i < ROWS; i++)
	{
		...
		{
			this->graf[i][j] = graf[i][j] != 0;
		}
	}
}

Search Board::findPath(Point start, Point goal)
{
	return Search(this, start, goal);
}
...
void Board::draw(Search search, Point currentStep)
{
